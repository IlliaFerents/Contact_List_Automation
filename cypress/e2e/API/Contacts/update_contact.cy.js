...
describe("Contact Update", () => {
  ...
  it(
    "returns error messages for updating existing contact with missing required fields",
    { tags: ["@api"] },
    function () {
      cy.getContactByID(this.contactID).then((response) => {
        const originalContactData = response.body;
  ...
  it(
    "returns error messages for updating existing contact with invalid data",
    { tags: ["@api"] },
    function () {
      cy.getContactByID(this.contactID).then((response) => {
        const originalContactData = response.body;
  ...
  it("returns error messages for request with invalid keys", { tags: ["@api"] }, function () {
    cy.getContactByID(this.contactID).then((response) => {
      const originalContactData = response.body;
