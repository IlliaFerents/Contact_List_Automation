...
describe("User Creation", () => {
  context("POST /users", () => {
    ...
    it("creates a user and logs in", { tags: ["@smoke", "@api"] }, function () {
    ...
    it("error when creating user with same email second time", { tags: ["@api"] }, function () {
      cy.addUser(this.validPayload).then((response) => {
        expect(response.status).to.eq(201);
        cy.addUser(this.validPayload).then((response) => {
    ...
    it("error when creating user with missing required fields", { tags: ["@api"] }, function () {
      cy.addUser({
        email: this.validPayload.email,
      }).then((response) => {
        expect(response.status).to.eq(400);
    ...
    it("error when creating user with missing email", { tags: ["@api"] }, function () {
      cy.addUser({
        firstName: this.validPayload.firstName,
        lastName: this.validPayload.lastName,
        password: this.validPayload.password,
    ...
    it("error when creating user with invalid data(email)", { tags: ["@api"] }, function () {
    ...
    it("error when creating user with invalid key", { tags: ["@api"] }, function () {
